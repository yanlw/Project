...
<script type="text/javascript">
	...
	function check_bankcard(){
		...
		$("#bankcard1_msg").text("银行卡号码已输入").addClass("right_msg");
	...
	function check_all() {
		...
		fileType2 = file2.substr(file2.indexOf(".") + 1).replace(/[ ]/g, "");
		if (!(fileType2 == "png" || fileType2 == "bmp" || fileType2 == "jpg" || fileType2 == "gif" || fileType2 == "jpeg")) {
			alert("身份证--照片文件类型不对！");
			return;
		}
...
<![endif]-->
	...
	<body>
		...
		<div class="cnt_z">
			...
			<form id="detailRegister" action="detailRegister.do?type=public"  method="post" enctype="multipart/form-data">
			<div class="wr_z">
				<div class="cnt_z_info">
					...
					<input type="hidden" name="ad_member_type" id="ad_member_type" value="0">
				</div>
				<div class="mod_title">
					<i class="icon iconfont icont_num">&#xe602;</i>营业执照上传
				</div>
				<div class="notice">
					照片内容真实有效，不得做任何修改；
					<br>
					支持jpg，bmg，gif格式照片，大小不超过2M
					<div class="fileInput">
                     <input type="file" name="imgFile2" id="imgFile2" class="upfile"/>
                 </div> 
				</div>
				<img src="../images/zhao.jpg" width="280" height="184" id="zhao">
				<div class="mod_title mg_bt_10">
					<i class="icon iconfont icont_num">&#xe603;</i>运营者信息
				</div>
				<div class="fo">
					<label>
						运营者姓名：
					...
					<input type="text" id="ad_member_name" name="ad_member_name"
						onblur="check_memberName();" onfocus="set_msg('name_msg','信息审核成功后身份证姓名不可修改；如果名字包含分隔号“·”，请勿省略。')">
				...
				<div class="fo">
					...
					<input type="text" id="mes" name="phoneCode" value="请输入手机短信收到的6位验证码"
						onfocus="value=''" onblur="check_mes();">
		...
		$(function() {
		    $('#file_upload').uploadify({
		        ...
		        'onUploadSuccess' : function(file, data, response) {//每次成功上传后执行的回调函数，从服务端返回数据到前端
		               ...
		               /*i++;*/
					   setTimeout(function(){$('#zhao').attr('src','./uploads/'+file.name)},1500)
		        ...
		        'onUploadSuccess' : function(file, data, response) {//每次成功上传后执行的回调函数，从服务端返回数据到前端
		               img_id_upload[i]=data;
		               /*i++;*/
					   setTimeout(function(){$('#identification').attr('src','./uploads/'+file.name)},1500)
